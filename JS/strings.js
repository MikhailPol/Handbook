/*
В JavaScript строки (strings) являются последовательностью символов, которые используются для хранения и представления текстовых данных. Они могут содержать любые символы, включая буквы, цифры, специальные символы и пробельные символы.

Создание строки: Строки в JavaScript можно создавать, заключая текст в одинарные (`'`) или двойные (`"`) или обратные(`'` - шаблонне строки) кавычки.

Строки могут быть объединены вместе с помощью оператора + или применения шаблонной строки. Это называется конкатенацией.

Конкретные символы в строке можно получить, обращаясь к ним по индексу в квадратных скобках (`[]`). Индексация начинается с 0.

Строки в JavaScript являются неизменяемыми (immutable), что означает, что операции над строками создают новые строки, а не изменяют существующие.
*/
let exampleStr = 'abc';
let exampleStr1 = "def";
let exampleStr2 = `${exampleStr} ${exampleStr1}` //Пример использования шаблонной строки
// Свойства
