...
Объекты. Основные понятия. Создание. Методы. Дескрипторы св-в объекта. prototype ,__proto__, getters, setrers;
...
В основном свойства объекта делятся на 2 типа:
Свойства данных (Data properties): Это обычные свойства, которые хранят значение данных. Они имеют ключ и значение.

Свойства аксессоры (Accessor properties): Это специальные свойства, которые используются для контроля доступа к другим свойствам объекта. Они состоят из геттера и сеттера, которые позволяют получать и устанавливать значения свойства.
...
// console.log(Object.getPrototypeOf(person) === personProto); //true
/*
5) Getters, setters (get, set).
В JavaScript есть возможность определить специальные методы get и set, которые позволяют управлять доступом к свойствам объекта. Они называются геттеры и сеттеры соответственно.
  1.Геттер (`get`): Это метод, который используется для получения значения свойства объекта. Он вызывается каждый раз, когда происходит попытка доступа к свойству объекта. Определяется с использованием ключевого слова get, за которым следует имя свойства.
  Использование свойства-аксессора get позволяет определить специальное поведение при получении значения свойства объекта.
*/
// const person = {
//   name: 'Mik',
//   surname: 'Ivanov',
...
//   }
// }
// console.log(person.fullPersonName); //Mik Ivanov
/*
  2.Сеттер (set) - Использование свойства-аксессора set позволяет определить специальное поведение при установке значения свойства объекта.
*/
// const person = {
//   name: '',
//   surname: '',
...
//   }
// }
// console.log(person.setName = 'Mik Ivanov'); //Mik Ivanov
// console.log(person); //{ name: 'Mik', surname: 'Ivanov', setName: [Setter] }